...
```
pipeline predict-model --model-type=tensorflow --model-name=mnist --model-tag=v1.3 --predict-server-url=http://localhost:6969 --test-request-path=./models/tensorflow/mnist/data/test_request.json --test-request-concurrency=100
```
...
curl -X POST -H "Content-Type: application/json" \
  ...
  http://localhost:6969/api/v1/model/predict/tensorflow/mnist/v1.3 \
  -w "\n\n"
...
## Stop Model Server
```
pipeline server-stop --model-type=tensorflow --model-name=mnist --model-tag=v1.3
```
