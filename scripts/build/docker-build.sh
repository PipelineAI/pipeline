...
cd $PIPELINE_HOME/package.ml/tensorflow/2a48110-4d0a571/ && sudo docker build -q -t fluxcapacitor/package-tensorflow-2a48110-4d0a571:$1 .
...
# prediction.ml
cd $PIPELINE_HOME/prediction.ml/jvm && sudo docker build -q -t fluxcapacitor/prediction-jvm:$1 .
cd $PIPELINE_HOME/prediction.ml/python3 && sudo docker build -q -t fluxcapacitor/prediction-python3:$1 .
...
# presto.ml
cd $PIPELINE_HOME/presto.ml/master && sudo docker build -q -t fluxcapacitor/presto-master-0.167:$1 .
cd $PIPELINE_HOME/presto.ml/worker && sudo docker build -q -t fluxcapacitor/presto-worker-0.167:$1 .
cd $PIPELINE_HOME/presto.ml/ui && sudo docker build -q -t fluxcapacitor/presto-ui:$1 .
