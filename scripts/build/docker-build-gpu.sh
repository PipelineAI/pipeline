...
cd $PIPELINE_HOME/package.ml/tensorflow/072355e-a44fd6d/ && sudo docker build $2 -t fluxcapacitor/package-tensorflow-072355e-a44fd6d-gpu:$1 -f Dockerfile.gpu .
cd $PIPELINE_HOME/package.ml/tensorflow/072355e-a44fd6d/ && sudo docker build $2 -t fluxcapacitor/package-tensorflow-072355e-a44fd6d-gpu-no-avx:$1 -f Dockerfile.gpu.no-avx .

cd $PIPELINE_HOME/clustered.ml/tensorflow && sudo docker build $2 -t fluxcapacitor/clustered-tensorflow-gpu:$1 -f Dockerfile.gpu .
cd $PIPELINE_HOME/gpu.ml && sudo docker build $2 -t fluxcapacitor/gpu-tensorflow-spark:$1 -f Dockerfile.gpu .
cd $PIPELINE_HOME/gpu.ml && sudo docker build $2 -t fluxcapacitor/gpu-tensorflow-spark-no-avx:$1 -f Dockerfile.gpu-no-avx .
