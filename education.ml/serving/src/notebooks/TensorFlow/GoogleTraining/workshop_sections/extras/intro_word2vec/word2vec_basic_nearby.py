...
with graph.as_default():
  ...
  # extra: add the ability to eval similarity for a specific word
  test_word = b'government'
  test_word_idx = dictionary[test_word]
  print("found word {} at index {}".format(test_word, test_word_idx))
  extra_embeddings = tf.nn.embedding_lookup(
      normalized_embeddings, [test_word_idx])
  extra_similarity = tf.matmul(
      extra_embeddings, normalized_embeddings, transpose_b=True)
...
with tf.Session(graph=graph) as session:
  ...
  for step in xrange(num_steps):
    ...
    if step % 10000 == 0:
      ...
      # Extra: eval the 'additional word' similarity
      esim = extra_similarity.eval()
      nearest = (-esim[0, :]).argsort()[1:top_k + 1]
      log_str = "Nearest to {}:".format(test_word)
