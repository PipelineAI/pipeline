...
class ModelPredictPython3Handler(tornado.web.RequestHandler):
    ...
    def get_model_assets(self, model_key_list):
        ...
        if model_key in ModelPredictPython3Handler._registry:
            ...
            try:
                artifact_name = 'pio_model'
                spec = importlib.util.spec_from_file_location(artifact_name, '/root/model/pio_model.py') 
...
def main():
    try:
        ...
        if not (options.PIO_MODEL_TYPE
            ...
            LOGGER.error('--PIO_MODEL_TYPE and --PIO_MODEL_NAME and \
                          --PIO_MODEL_SERVER_PORT and --PIO_MODEL_SERVER_PROMETHEUS_PORT and \
                          --PIO_MODEL_SERVER_TENSORFLOW_SERVING_PORT and must be set')
            return
