...
class ModelPredictPython3NameOnlyHandler(tornado.web.RequestHandler):
    ...
    def get(self, model_name):
        try:
            models = ModelPredictPython3NameOnlyHandler._registry
            self.write(models)
            self.finish()
        except Exception as e:
    ...
    def get_model_assets(self, model_key_list):
        model_key = '/'.join(model_key_list)
        if model_key in ModelPredictPython3NameOnlyHandler._registry:
            model = ModelPredictPython3NameOnlyHandler._registry[model_key]
        else:
...
class ModelPredictScikitNameOnlyHandler(tornado.web.RequestHandler):
    ...
    def get(self, model_name):
        try:
            self.write(json.dumps(ModelPredictScikitNameOnlyHandler._registry.keys()))
            self.finish()
        except Exception as e:
            logging.exception('ModelPredictScikitNameOnlyHandler.get: Exception {0}'.format(str(e)))
    ...
    def get_model_assets(self, model_key_list):
        model_key = '/'.join(model_key_list)
        if model_key in ModelPredictScikitNameOnlyHandler._registry:
            model = ModelPredictScikitNameOnlyHandler._registry[model_key]
        else:
            LOGGER.info('Model Server get_model_assets if: begin')
            with REQUEST_LATENCY_BUCKETS.labels('pickle-load', *model_key_list).time():
                ...
                try:
                    ...
                    ModelPredictScikitNameOnlyHandler._registry[model_key] = model
...
class ModelDeployScikitNameOnlyHandler(tornado.web.RequestHandler):
