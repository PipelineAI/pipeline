...
(function (VZ) {
    ...
    (function (Sorting) {
        ...
        describe('compareTagNames', function () {
            var sortTagNames = function (a) { return a.sort(Sorting.compareTagNames); };
            it('is asciibetical', function () {
                assert.deepEqual(sortTagNames(['a', 'b']), ['a', 'b']);
                assert.deepEqual(sortTagNames(['a', 'B']), ['B', 'a']);
            });
            it('sorts integer portions', function () {
            ...
            it('sorts fixed point numbers', function () {
                assert.deepEqual(sortTagNames(['a0.1', 'a0.01']), ['a0.01', 'a0.1']);
            });
            it('sorts engineering notation', function () {
                ...
                assert.deepEqual(sortTagNames(['a1.0e+9', 'a9.0e+8']), ['a9.0e+8', 'a1.0e+9']);
            });
            it('is componentized by slash', function () {
                assert.deepEqual(['a+/a', 'a/a', 'ab/a'].sort(), ['a+/a', 'a/a', 'ab/a']);
                assert.deepEqual(sortTagNames(['a+/a', 'a/a', 'ab/a']), ['a/a', 'a+/a', 'ab/a']);
            });
            it('is componentized by underscore', function () {
                assert.deepEqual(sortTagNames(['a+_a', 'a_a', 'ab_a']), ['a_a', 'a+_a', 'ab_a']);
                assert.deepEqual(sortTagNames(['a+/a', 'a_a', 'ab_a']), ['a_a', 'a+/a', 'ab_a']);
            });
            it('is componentized by number boundaries', function () {
                assert.deepEqual(sortTagNames(['a+0a', 'a0a', 'ab0a']), ['a0a', 'a+0a', 'ab0a']);
            });
            it('empty comes first', function () {
                assert.deepEqual(sortTagNames(['a', '//', '/', '']), ['', '/', '//', 'a']);
            });
            it('decimal parsed correctly', function () {
