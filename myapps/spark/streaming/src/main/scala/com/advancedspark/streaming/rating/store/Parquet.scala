  ...
  def main(args: Array[String]) {
    ...
    ratingsStream.foreachRDD {
      (message: RDD[(String, String)], batchTime: Time) => {
        ...
        val ratingsDF = message.map(_._2.split(",")).map(rating => Rating(rating(0).trim.toInt, rating(1).trim.toInt, rating(2).trim.toInt, rating(3).trim.toString, batchTime.milliseconds)).toDF("userId", "itemId", "rating", "geo", "timestamp")
