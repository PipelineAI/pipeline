...
cd $PIPELINE_HOME
x#nohup $SPARK_HOME/sbin/start-slave.sh --cores $SPARK_WORKER_CORES --memory $SPARK_WORKER_MEMORY --webui-port 6061 -h 0.0.0.0 spark://$SPARK_MASTER_HOST:$SPARK_MASTER_PORT
#nohup $SPARK_HOME/sbin/start-slave.sh --cores 8 --memory 50g --webui-port 6061 -h 0.0.0.0 spark://<master>
